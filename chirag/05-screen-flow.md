...
* **Key Features:** Group alerts by transaction ID, status transition controls (`OPEN` $\rightarrow$ `ACKNOWLEDGED` $\rightarrow$ `INVESTIGATING` $\rightarrow$ `CLOSED`), automated trigger reason breakdown.
